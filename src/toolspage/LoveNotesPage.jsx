...
const FloatingHeart = styled(Box)(({ theme, position, size, opacity }) => ({
  ...
  width: `${size}px`,
  height: `${size}px`,
  background: `radial-gradient(circle, ${COLORS.primary} 0%, ${
    COLORS.primary + "80"
  } 100%)`,
  borderRadius: "50%",
  opacity: opacity,
  ...
  const saveNote = async () => {
    ...
    try {
      ...
      const noteData = {
        ...
        createTime:
          selectedNote.date?.toISOString() || new Date().toISOString(),
        coupleId: coupleId,
      };
      ...
      if (result && result.code === "200") {
        ...
        setError(
          result?.message || (isNewNote ? "创建笔记失败" : "更新笔记失败")
        );
        setOpenSnackbar(true);
      }
    } catch (error) {
          ...
          >
            <Box>
              ...
              <span style={{ fontWeight: "500" }}>
                {
                  getUser(selectedNote?.authorId || user?.userId || "user1")
                    .name
                }
              </span>
            </Box>
            <Box>
              ...
              />
              <span style={{ fontWeight: "500" }}>
                {formatDate(selectedNote?.date || new Date())}
              </span>
            </Box>
          </Box>
        </DialogContent>
